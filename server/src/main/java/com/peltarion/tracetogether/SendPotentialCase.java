...
public class SendPotentialCase
{
	public static void main(String[] args)
	{
		...
		long valueFromBluetoothScanner = 3;
		long myId = 1;
		...
		AdminUser adminUser = AdminUser.newBuilder().setUser(user).setPassword(Password.newBuilder().setPassword(password).build()).build();
		...
		stub.confirmedCase(PotentialCases.newBuilder().setConfirmedId(Id.newBuilder().setId(myId).build()).addPotentialCases(potentialCase).build());
